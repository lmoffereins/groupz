...
/**
 * Groupz extend Simple History Extender
 *
 * Extend Simple History for Groupz events
 * 
...
class Groupz_Simple_History extends SH_Extend {
	...
	function extend_group( $group_id, $action ){
		$this->extend( array( 
			'action' => $action,
	...
	function create_group( $group_id ){
	 ...
	 */
	function added_post_group( $post_id, $group_id ){
		$this->extend_post( $post_id, sprintf( $this->events['add'], get_group_name( $group_id ) ) );
	}

	function removed_post_group( $post_id, $group_id ){
	...
	function add_post_edit_group( $post_id, $group_id ){
		// Don't log revisions
	...
	function remove_post_edit_group( $post_id, $group_id ){
		// Don't log revisions
		if ( wp_is_post_revision( $post_id ) )
			return;
	...
	function updated_post_edit_groups( $post_id, $new_group_ids ){
		// Don't log revisions
		if ( wp_is_post_revision( $post_id ) )
			return;
	...
	function remove_post_edit_groups( $post_id ){
